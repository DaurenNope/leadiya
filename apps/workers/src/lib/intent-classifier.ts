...
/** Short skeptical / objection — route to question + LLM or scripted handling */
const OBJECTION = ['зачем', 'why', 'what for', 'какой смысл', 'нафига', 'зачем вы', 'why are you']
...
function keywordMatch(text: string): Intent | null {
  ...
  if (OBJECTION.some(k => lower.includes(k)) || /\bзачем\b/i.test(text)) return 'question'
