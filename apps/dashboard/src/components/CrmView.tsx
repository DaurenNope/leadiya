...
type Props = {
  ...
  onSeedConsumed: () => void
  onBrowseLeads: () => void
  /** Открыть раздел «WhatsApp»; можно передать лид/телефон чтобы выделить диалог в ленте */
  onOpenWhatsApp?: (opts?: { leadId?: string; phoneDigits?: string }) => void
}
              ...
              <header className="shrink-0 px-5 sm:px-6 py-4 border-b border-white/[0.06] bg-slate-950/40">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {onOpenWhatsApp && waDigitsFromField ? (
                      <button
                        type="button"
                        onClick={() =>
                          onOpenWhatsApp({ leadId: selected.leadId, phoneDigits: waDigitsFromField })
                        }
                    ...
                    {waQuick ? (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center rounded-xl border border-white/[0.12] px-2.5 py-2 text-[11px] font-medium text-slate-400 hover:text-slate-200 hover:bg-white/[0.04]"
                        title="Открыть чат в приложении WhatsApp (вне браузера Leadiya)"
                      >
                        wa.me
                      </a>
                    ) : null}
